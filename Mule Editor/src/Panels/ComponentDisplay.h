...
static Mule::Ref<Mule::ModelDesc> LoadMeshAsync(std::string path, Mule::Entity e)
{
	Mule::AssetManager& manager = Mule::AssetManager::Get();
	Mule::Ref<Mule::ModelDesc> model = Mule::LoadModel(path);
	manager.InsertAsset(model);
	e.GetComponent<Mule::ModelComponent>().ModelHandle = model->Handle();
	return model;
}
